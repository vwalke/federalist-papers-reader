...
- Gazette prose enables font-supported common and historical ligatures and uses source-attested `fœderal` rendering; the essay heading likewise reads `THE FŒDERALIST.` in Gazette, matching the social preview cards. Reader mode normalizes both to the `federal` forms, and screen readers always hear the modern spelling.
...
Every paper has its own 1200×630 link-preview card echoing the sheet: the masthead nameplate, the paper's number as the hero (`THE FŒDERALIST. No. X.`), its title, and `federalistreader.org` on warm newsprint. Cards are raster (JPEG, since no major platform renders SVG previews), generated by `pnpm generate:social-cards` from the self-hosted fonts and committed under `public/social-cards/`; the home, About, and fallback pages share `default.jpg`. Pages emit absolute `og:image`/`twitter:image` URLs with `twitter:card = summary_large_image` so shares render as a full banner.
